...
def train_vae(model, dataloader, optimizer, num_epochs=10, device='cpu', latent_dim=20):
    model.to(device)
    model.train()
    ...
    for epoch in tq:
        ...
        # 🔹 每个 epoch 保存采样结果
        save_samples(model, latent_dim, device, epoch)
...
train_vae(vae, dataloader, optimizer, num_epochs, device, latent_dim)
