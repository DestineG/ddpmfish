...
class VAEEncoder(torch.nn.Module):
    ...
    def forward(self, x, label=None):
        if label is not None:
            one_hot = F.one_hot(label, num_classes=10).float()
...
def train_vae(model, dataloader, optimizer, num_epochs=10, device='cpu', latent_dim=20, exp_dir="exps/vae_base"):
    ...
    # 子图1: 总损失
    axes[0, 0].plot(epochs, history['total'], color='black', linewidth=1.5)
    axes[0, 0].set_title("Total Training Loss")
    
    # 子图2: 重构损失
    axes[0, 1].plot(epochs, history['recon'], color='blue', linewidth=1.5)
    axes[0, 1].set_title("Reconstruction Loss (BCE)")
    
    # 子图3: KL 散度
    axes[1, 0].plot(epochs, history['kl'], color='red', linewidth=1.5)
    axes[1, 0].set_title("KL Divergence")
    
    # 子图4: 分类损失
    axes[1, 1].plot(epochs, history['class'], color='green', linewidth=1.5)
...
if __name__ == "__main__":
    ...
    device = 'cpu'
    exp_dir = "exps/vae_base_control_v3"
